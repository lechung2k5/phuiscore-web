...
// Insert the helper after THEME_COLORS
content = content.replace(/const blinkStyles =/g, helperCode + '\nconst blinkStyles =');

// Re-replace the ugly fallbacks with getSafeLogo
// Old string: <IMG src={!league.logo ? "https://www.sofascore.com/static/images/placeholders/tournament.png" : (league.logo.startsWith('http') ? league.logo : `${API_BASE_URL}${league.logo}`)} width={18} height={18} style={{ objectFit: 'contain' }} />

// We can just use regex to replace all `<IMG src={!...}` to `<IMG src={getSafeLogo(...)}`
...
console.log('Update getSafeLogo injected successfully.');
