...
export const CreateTeamModal = ({ open, setOpen, onSuccess }: { open: boolean, setOpen: (o: boolean) => void, onSuccess: () => void }) => {
  ...
  const handleSubmit = async () => {
    if (!formData.name || !formData.leader || !province) {
      alert("Vui lòng nhập Tên đội, Đội trưởng và Tỉnh/Thành phố")
      return
    }
    setLoading(true)
    try {
      const token = localStorage.getItem('token')
      const finalArea = areaDetail ? `${areaDetail}, ${province}` : province
      await axios.post(`${API}/teams`, { ...formData, area: finalArea }, {
      ...
      console.error(error)
      alert("Lỗi khi tạo đội. Vui lòng thử lại.")
    } finally {
      setLoading(false)
    }
  }
    ...
    <Sheet modal open={open} onOpenChange={setOpen} snapPoints={[90]} dismissOnSnapToBottom position={0}>
      ...
      <Sheet.Frame backgroundColor={C.bgDark as any} padding="$0" borderTopLeftRadius={24} borderTopRightRadius={24}>
        ...
        <XStack padding="$4" borderBottomWidth={1} borderColor={C.borderDark as any} justifyContent="space-between" alignItems="center" backgroundColor={C.cardDark as any} borderTopLeftRadius={24} borderTopRightRadius={24}>
          <Text color="white" fontSize={18} fontWeight="800">Tạo Đội Bóng Mới</Text>
        ...
        <ScrollView padding="$5" flex={1}>
          <YStack maxWidth={600} marginHorizontal="auto" width="100%" gap="$5" paddingBottom="$10">
            ...
            <YStack alignItems="center" gap="$3">
              <View position="relative" width={100} height={100} borderRadius={50} overflow="hidden" backgroundColor="rgba(255,255,255,0.05)" borderWidth={2} borderColor={formData.logo_url ? C.primary : ("rgba(255,255,255,0.1)" as any)} alignItems="center" justifyContent="center">
                {formData.logo_url ? (
              ...
              <input type="file" id="team_logo_upload" style={{ display: 'none' }} accept="image/*" onChange={handleUploadLogo} />
            </YStack>
            ...
            <XStack gap="$4" flexWrap="wrap">
              ...
              <YStack gap="$2" flex={1} minWidth={250}>
                <Text color={C.textGray as any} fontSize={14} fontWeight="600">Tỉnh / Thành phố <Text color="#e74c3c">*</Text></Text>
                <select
                  value={province}
                ...
                >
                  <option value="" disabled style={{ color: 'white', backgroundColor: '#1a1f1c' }}>Chọn Tỉnh/Thành phố</option>
                  {PROVINCES_34.map(p => <option key={p} value={p} style={{ color: 'white', backgroundColor: '#1a1f1c' }}>{p}</option>)}
                </select>
              </YStack>
            </XStack>

            <YStack gap="$2">
              <Text color={C.subText as any} fontSize={14} fontWeight="600">Vị trí chi tiết (Quận/Huyện, Sân nhà...)</Text>
              <Input backgroundColor="transparent" borderWidth={1} borderColor={C.borderDark as any} color="white" placeholder="VD: Sân Chảo Lửa, Q.Tân Bình" value={areaDetail} onChangeText={setAreaDetail} />
            </YStack>
            ...
            <Button marginTop="$4" size="$5" backgroundColor={C.primary as any} onPress={handleSubmit} disabled={loading} opacity={loading ? 0.7 : 1} icon={loading ? <Spinner color="white" /> : <CheckCircle2 color="white" />}>
              <Text color="white" fontWeight="800" fontSize={16}>{loading ? 'Đang xử lý...' : 'HOÀN TẤT TẠO ĐỘI'}</Text>
            </Button>
