...
export const EditMemberModal = ({ 
  ...
  // Cập nhật form data khi member thay đổi
  useEffect(() => {
    if (member && open) {
      setFormData({
        name: member.name || '',
        shirtNumber: member.shirtNumber ? String(member.shirtNumber) : '',
  ...
  const handleSubmit = async () => {
    if (!formData.name) {
      onAlert("Vui lòng nhập Tên cầu thủ")
      return
    }
    setLoading(true)
    try {
      ...
      await axios.put(`${API}/team-members/${memberId}`, payload, {
        headers: { Authorization: `Bearer ${token}` }
      })
      onAlert("Đã cập nhật cầu thủ!")
      setOpen(false)
      onSuccess()
    } catch (e: any) {
      console.error(e)
      const errMsg = e.response?.data?.message || e.message || 'Không thể cập nhật'
      onAlert("Lỗi cập nhật: " + errMsg)
    } finally {
      setLoading(false)
    }
  }
    ...
    <Sheet modal open={open} onOpenChange={setOpen} snapPoints={[85]} dismissOnSnapToBottom position={0}>
      ...
      <Sheet.Frame backgroundColor={C.bgDark as any} padding="$0" borderTopLeftRadius={24} borderTopRightRadius={24}>
        ...
        <XStack padding="$4" borderBottomWidth={1} borderColor={C.borderDark as any} justifyContent="space-between" alignItems="center" backgroundColor={C.cardDark as any} borderTopLeftRadius={24} borderTopRightRadius={24}>
          <Text color="white" fontSize={18} fontWeight="800">Sửa thông tin Cầu thủ</Text>
        ...
        <ScrollView padding="$5" flex={1}>
          <YStack maxWidth={500} marginHorizontal="auto" width="100%" gap="$5" paddingBottom="$10">
            ...
            <YStack alignItems="center" gap="$3">
              ...
              <Button size="$3" backgroundColor="rgba(255,255,255,0.1)" hoverStyle={{ backgroundColor: 'rgba(255,255,255,0.2)' } as any} icon={<Upload size={16} color="white" />} onPress={() => document.getElementById('edit_player_avatar_upload')?.click()}>
                <Text color="white" fontWeight="600">Thay ảnh đại diện</Text>
              </Button>
              <input type="file" id="edit_player_avatar_upload" style={{ display: 'none' }} accept="image/*" onChange={handleUploadAvatar} />
            </YStack>
            ...
            <Button marginTop="$4" size="$5" backgroundColor={C.primary as any} onPress={handleSubmit} disabled={loading} opacity={loading ? 0.7 : 1} icon={loading ? <Spinner color="white" /> : <CheckCircle2 color="white" />}>
              <Text color="white" fontWeight="800" fontSize={16}>{loading ? 'Đang lưu...' : 'LƯU THAY ĐỔI'}</Text>
            </Button>
